...
- `STOCK_VERDICT_ENABLED=false` → 🎯 軍師判讀 banner / 卡片短標 / 推播一行都不顯
...
ML_NEW_FEATURES_ENABLED=true       # v4 features(籌碼 / 多時間軸 / 產業相對強度)— off 退化 v3
...
STOCK_VERDICT_ENABLED=true         # 🎯 軍師判讀(整合 K 線形態+警示+regime+ML+共識+題材+持倉)→ 🟢/🟡/🔴
